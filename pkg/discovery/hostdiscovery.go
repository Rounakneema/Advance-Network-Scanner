...
func (hd *HostDiscoverer) PingHosts(ctx context.Context, targets []net.IP, timeout time.Duration) ([]*models.Host, error) {
	output.PrintInfo("Starting ICMP host discovery (may require root/admin on some OSes)...")

	var liveHosts []*models.Host
	var wg sync.WaitGroup
	hostChan := make(chan *models.Host, len(targets))
	...
	// Create a local context to signal the read loop to stop when this function exits
	...
	go func() {
		...
		for {
			...
			default:
				...
				if err != nil {
					// Check for context cancellation or closed connection to exit cleanly
					if ctx.Err() != nil {
						return
					}
					...
					// Silence "closed network connection" errors which happen on cleanup
					...
					if echoReply, ok := msg.Body.(*icmp.Echo); ok {
						...
						if targetIP, loaded := sentICMPs.LoadAndDelete(key); loaded {
							if targetIP.(net.IP).Equal(peer.(*net.IPAddr).IP) {
								host := models.NewHost(peer.(*net.IPAddr).IP)
								host.IsUp = true
								hostChan <- host
							}
						}
...
func (hd *HostDiscoverer) TCPProbeHosts(ctx context.Context, targets []net.IP, timeout time.Duration) ([]*models.Host, error) {
...
func (hd *HostDiscoverer) ARPScan(ctx context.Context, targetCIDR *net.IPNet, timeout time.Duration) ([]*models.Host, error) {
	output.PrintInfo("Starting ARP host discovery for %s (requires root/admin)...", targetCIDR.String())

	var liveHosts []*models.Host
	var wg sync.WaitGroup
	hostChan := make(chan *models.Host, 100)
...
// Requires root/admin privileges for pcap handle.
func (hd *HostDiscoverer) TCPHostProbePing(ctx context.Context, targets []net.IP, timeout time.Duration) ([]*models.Host, error) {
	...
	// Struct to pass response data with TTL
				...
				if tcpLayer := packet.Layer(layers.LayerTypeTCP); tcpLayer != nil {
					...
					if ipLayer := packet.Layer(layers.LayerTypeIPv4); ipLayer != nil {
						...
						if ch, loaded := synCorrelation.Load(key); loaded {
							...
							ch.(chan probeResponse) <- probeResponse{isLive: isLive, ttl: ip4.TTL}
	...
	for i, ip := range targets {
		...
		case sem <- struct{}{}:
			...
			go func(ip net.IP, idx int) {
				...
				responseChan := make(chan probeResponse, 1)
