...
import (
	...
	// REMOVED: "revealr/pkg/core" - PrintScanSummary is moved, so core types are no longer needed here.
)
...
func PrintHostScanProgress(current, total int, target string) {
	stdoutMux.Lock()
	defer stdoutMux.Unlock()
	fmt.Printf("\r%s[%s] [%s] Discovering hosts: %d/%d - %s...%s", Cyan, time.Now().Format("15:04:05"), "INFO", current, total, target, Reset)
	if current == total {
		fmt.Println()
	}
}
...
// PrintOpenPort is now ONLY used for Go's *initial, confident* service identification.
// Python's enrichment will be shown in the final summary.
func PrintOpenPort(host, protocol string, port int, service string) {
	stdoutMux.Lock()
	defer stdoutMux.Unlock()
	// Using fmt.Fprintf(os.Stdout, ...) to ensure it prints to console immediately.
	// This will not overwrite the progress line, it will print above it.
	fmt.Fprintf(os.Stdout, "%s[%s] [OPEN] %s:%d/%s - %s%s\n", Yellow, time.Now().Format("15:04:05"), host, port, protocol, service, Reset)
}

// REMOVED: PrintScanSummary - This function has been moved to pkg/app/orchestrator.go to break import cycle.